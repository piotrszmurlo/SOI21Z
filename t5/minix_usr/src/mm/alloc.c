...
PRIVATE int algorithm_choice = 0;
 ...
 */
  ...
  register struct hole *hp, *prev_ptr, *worst, *prev_worst_ptr;
  ...
  return(NO_MEM);
    ...
    worst->h_len -= clicks;
    worst->h_base += clicks;
    if (worst->h_len == 0) del_slot(prev_worst_ptr, worst);
    return old_base;
  } while (swap_out());		/* try to swap some other process out */
  return(NO_MEM);
  }
...
int do_hole_map(void) {
  ...
  phys_clicks nbytes = mm_in.m1_i1;
  phys_clicks user_buffer = (phys_clicks)mm_in.m1_p1;
  struct hole *hp = hole_head;
  int hole_count, i;
  hole_count = ((nbytes/sizeof(phys_clicks) - 1) / 2);
  i = 0;

	while (hp != NIL_HOLE && hp->h_base < swap_base && hole_count-- > 0) {
    buffer[i++] = hp->h_len;
    buffer[i++] = hp->h_base;
    hp = hp->h_next;
  }
  buffer[i] = 0;
  sys_copy(MM_PROC_NR, D,(phys_clicks)buffer, who, D, user_buffer, nbytes);
  return i/2;
}

int do_worst_fit (void) {
  if (mm_in.m1_i1 == 0 || mm_in.m1_i1 == 1){
    algorithm_choice = mm_in.m1_i1;
    return 0;
  }
  return -1;
}
